...
def is_valid_token(tok):
    ...
    if tok in STOPWORDS_SET:
        if config.PROCESS_TEXT_DEBUGGER or config.DEBUG_ALL:
            print(f"***Token \"{tok}\" failed filtration check due to: IS STOPWORD***")
        return False
    # Token length must be at least (ie. >=) the minimum length.
    if len(tok) < config.MIN_TOK_LEN:
        if config.PROCESS_TEXT_DEBUGGER or config.DEBUG_ALL:
            print(f"***Token \"{tok}\" failed filtration check due to: LENGTH < {config.MIN_TOK_LEN}***")
        return False
...
def process_text(raw_text):
    ...
    # 3rd: List comprehension to filter tokens to only those that pass the checks in is_valid_token().
    filtered_tokens = [
    ...
    if config.PROCESS_TEXT_DEBUGGER or config.DEBUG_ALL:
        print(f"***FILTERED TOKENS FROM PROCESS_TEXT():***\n\"{filtered_tokens}\"")

    # 4th: POS Tagging.
    # List comprehension to filter tokens to only those that match the desired POS.
    ...
    if config.PROCESS_TEXT_DEBUGGER or config.DEBUG_ALL:
        print(f"***FILTERED POS FROM PROCESS_TEXT():***\n\"{filtered_pos}\"")

    # 5th: List comprehension to lemmatize the filtered tokens, using POS information to get more accurate lemmatization.
    all_lemmas = [
        LEMMATIZER.lemmatize(word, config.POS_DICT[pos])   # Use dict mapping to convert pos_tag to WordNet tag.
        for word, pos 
        in filtered_pos]

    # 6th: List comprehension to validate that the final lemmatized words are at least the min length.
    filtered_lemmas = [
        lemma 
        for lemma 
        in all_lemmas 
    ...
    if config.PROCESS_TEXT_DEBUGGER or config.DEBUG_ALL:
        print(f"***FILTERED LEMMAS FROM PROCESS_TEXT():***\n\"{filtered_lemmas}\"")

    # 7th: Count frequencies of each lemma.
    # List comprehension to further filter lemmas only by those appearing "frequently".
    ...
    if config.PROCESS_TEXT_DEBUGGER or config.DEBUG_ALL:
        print(f"***FREQUENT LEMMAS FROM PROCESS_TEXT():***\n\"{frequent_lemmas}\"")
