...
def get_filename_from_url(url: str) -> str:
    ...
    # Remove language subdomains such as "en." from
    # Wikipedia and Wiktionary URLs.
    ...
    # Remove common filename prefixes/suffixes from
    # Gutenberg-style ebook URLs.
    path = re.sub(
        r"pg(\d+)-images$",
        r"\1",
        path,
...
def generate_text_file(seed_url: str, num_webpages: int, output_dir=config.TEXTS) -> str:
    ...
    filename = (
        f"{get_filename_from_url(seed_url)}"
        f"_{timestamp}.txt"
    )
